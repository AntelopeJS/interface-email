# Changelog


## v0.0.5

[compare changes](https://github.com/AntelopeJS/interface-email/compare/v0.0.4...v0.0.5)

### 🚀 Enhancements

- **skills:** Ship a consumer skill with the package ([#3](https://github.com/AntelopeJS/interface-email/pull/3))

### ❤️ Contributors

- Antony Rizzitelli <upd4ting@gmail.com>

## v0.0.4

[compare changes](https://github.com/AntelopeJS/interface-email/compare/v0.0.3...v0.0.4)

## v0.0.3

[compare changes](https://github.com/AntelopeJS/interface-email/compare/v0.0.2...v0.0.3)

### 📖 Documentation

- Rewrite documentation for production readiness ([4d1ed66](https://github.com/AntelopeJS/interface-email/commit/4d1ed66))

### 🏡 Chore

- Update package.json ([efd0f24](https://github.com/AntelopeJS/interface-email/commit/efd0f24))
- Update dependencies ([8a2e0d8](https://github.com/AntelopeJS/interface-email/commit/8a2e0d8))
- Package maintenance ([13cb56d](https://github.com/AntelopeJS/interface-email/commit/13cb56d))

### ❤️ Contributors

- Antony Rizzitelli <upd4ting@gmail.com>

## v0.0.2


### 🏡 Chore

- Initial commit ([1eb1f9f](https://github.com/AntelopeJS/interface-email/commit/1eb1f9f))
- Update lockfile for published interface-core dependency ([d8e0e1d](https://github.com/AntelopeJS/interface-email/commit/d8e0e1d))

### ❤️ Contributors

- Antony Rizzitelli <upd4ting@gmail.com>

